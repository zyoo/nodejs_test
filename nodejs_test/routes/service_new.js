                            ...
                            if(err) {
                                ...
                                conn.query('select role_name, unique_flag from role_unique where service_name=?', [serviceChoice], function(err, result) {
                                    if(err) {
                                        ...
                                        req.session.Service = serviceChoice;
                                        res.render('service', {title:'Service:Select Service', Flag:'true', Components:roleName, HostList:str_host_list, Unique_Flag:str_res, Config:'', err_info:'',username:req.session.username});
                                    }
                                });
                            } 
                        });                   
