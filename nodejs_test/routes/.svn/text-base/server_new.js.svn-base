...
exports.serverInfoUpdate=function(req, res) {
...
exports.server = function(req, res) {
    ...
    conn.query("select host, ip, cpu, ram, disk, os, load_info, status from host_info", function(err, result){
            if(err) {
                console.log(err);
                res.render('server',
							{title : 'Server:Add New Server',
									 flag : 'true',
									 CheckRes : 'database option failed',
									 DataBase_Res : '',
									 host_arg : ''});
            }
            else {
...
exports.addServerFirst = function(req, res) {
    ...
    if(host == null || port == null) {
        ...
        res.render('server', {title : 'Service:Add New Server', 
							  flag : 'true',
							  CheckRes : 'the input info is illegal', 
							  DataBase_Res : ''});
    }
	else {
		...
		step (
			function() {
				...
				exec(api_checkhost_cmd, group());
			},
			function(err, stdout, stderr) {
				if(err) {
					console.log(err);
					res.render('server', {title : "Server:Add New Server",
										  flag : 'true',
										  CheckRes : 'the input info for the host is illegal,the check err is ' + JSON.stringify(err) + stdout + '. please check the host info,then try again',
										  DataBase_Res : '',
										  host_arg : ''});
					return;
				}	
					...
					var api_gethostinfo_cmd = 'sh /usr/local/NArk/bin/HOST -g ' + host + ' -p ' + port + ' -k ' + new_path;
					exec(api_gethostinfo_cmd, group());
				}
			},
			function(err, stdout, stderr) {
				if(err) {
					...
					res.render('server', {title : 'Server:Add new server', 
										  flag : 'true',
										  CheckRes : 'get the host info error:' + stdout + '.please cancle the operation',
										  DataBase_Res : '',
										  host_arg : ''}); 		
					return;
				}
					...
					conn.query("delete from host_info_temp where host = ?", [host], group());
				}	
			},
			function(err,result) {
				if(err) {
					...
					var group = this.group();	
					conn.query("insert into host_info_temp set host = ?, ip = ?, cpu = ?, ram = ?, disk = ?, os = ?, load_info = ?, key_file_path = ?, port = ?, status = ?", [host, ipStr, list_host_info_res['cpu'], list_host_info_res['ram'], list_host_info_res['disk'], list_host_info_res['os'], list_host_info_res['load'], new_path, port, 'live'], group());
				}
			},
			function(err, result) {
				if(err) {
					...
					res.render('server', {title : 'Service:Add New Server',
										 flag : 'true',
										 CheckRes : 'update the database failed',
										 DataBase_Res : '',
										 host_arg : ''});			
				}	
				else {
					res.render('server', {title : 'Service:Add New Server',
										  flag : 'true',
										  CheckRes : 'the host is legal, please go on to add the server to the cluser',
										  DataBase_Res : '',
										  host_arg : host});
				}
			}	
...
exports.addServerSecond = function(req, res) {
    ...
    if(but_value == "true"){
        var current_path = process.cwd();
        var new_path = current_path + '/keyfile_dir/' + host + '.keyfile.list';
        var api_addhost_cmd='sh /usr/local/NArk/bin/HOST -a '+host+' -p '+port+' -k '+new_path;
		console.log(api_addhost_cmd);
		step (
			function() {
				var group = this.group();
				exec(api_addhost_cmd, group());
			},
			function(err, stdout, stderr) {
				if(err) {
					...
					res.render('server', {title : 'Server:Add New Server',
										  flag  : 'true',
										  CheckRes : 'add host failed,the error info is ' + JSON.stringify(err) + stdout,
										  DataBase_Res : '',
										  host_arg : ''});			
					return;
				}	
					...
					var group = this.group();	
					conn.query("insert into host_info select * from host_info_temp where host = ?", [host], group());
				}
			},
			function(err, result) {
				if(err) {
					...
					res.render('server',{title : 'Server:Add New Server',
										 flag : 'true',
										 CheckRes : 'add host failed, the error info is ' + JSON.stringify(err),
										 DataBase_Res : '',
										 host_arg : ''});			
					return;
				}	
			...
			function(err, result) {
				if(err) {
					...
					var group = this.group();	
					conn.query("select host, ip, cpu, ram, disk, os, load_info, status from host_info", group());
				}
			},	
			function(err,result) {
				if(err) {
...
function removeOneRoleFromTheHost(role, host, service, res, req) {
	uninstallRole(req, res, host, role, service, callBackForUninstallRole(req, res, host));
}

function callBackForRemoveRole(req, res, host) {
	return function() {
...
exports.getHostInfo = function(req, res) {
	...
	step (
		function() {
			conn.query('select * from host_info where host = ?', host, this.parallel());
			conn.query('select * from services_on_the_hosts_info where host = ?', host, this.parallel());
		},			
		function(err, hostInfo, roleInfo) {
			if(err) {
				console.log(err);	
				res.send(200, {error : "get server info from db failed"});
			}	
			else {
				if(hostInfo.length == 0) {
					console.log('empty result, the host is not exist');
					res.send(200, {error : "get server info from db failed"});
					return;
				}	
				if(roleInfo.length == 0) {
					console.log('empty result, no services on the host ');
				}
				//res.send(200, {host: hostInfo, role: roleInfo});	
				console.log(hostInfo);
				var hostInfoStr = JSON.stringify(hostInfo);
				console.log(hostInfoStr);
				console.log(roleInfo);
				var roleInfoStr = JSON.stringify(roleInfo);
				console.log(roleInfoStr);
				res.render('host_demo', {title: 'HostInfo',
									 	 host: host,
									 	 hostinfo: hostInfoStr,
